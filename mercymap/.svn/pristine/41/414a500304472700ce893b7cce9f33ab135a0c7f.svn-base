...
//调用方法
-(void)setLocationView:(MAMapView *)mapview selfView:(UIViewController*)selfView Category:(NSString *)catrgory Date:(int)dates How:(NSString *)how Who:(NSString *)who{
     comeView    = selfView;
...
-(void)getMapViewInfo{
    ...
    dic = @{@"DateCount":_dateNum,@"ShopCategoryIDs":_categoryStr,@"LocationArea":location,@"pageIndex":index,@"pageSize":pageSize,@"How":_how,@"Who":_who};
    [self serviceGetMapViewInfo:dic];
}

-(void)serviceGetMapViewInfo:(NSDictionary *)dic{
    MBProgressHUD *_Hud = [MBProgressHUD showHUDAddedTo:_mapView animated:YES];
    _Hud.labelText = @"加载中……";
    ...
    [_service getArrayData:dic Url:url Title:@"_IlistShopList" SuccessBlock:^(NSArray *modelArray) {
        ...
        [CommoneTools alertOnView:_mapView content:str];
    }];
}
