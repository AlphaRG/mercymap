...
-(void)setView{
    NSArray *titles = @[@"创建商家",@"扫一扫",@"我要收款"];
    NSArray *images =@[@"create_shop",@"saoyisao",@"collections"];
...
-(void)collectionsScan{
    if ([[[NSUserDefaults standardUserDefaults]objectForKey:@"UserLogin"]boolValue]){
        MM_CollectionsViewController *VC = [[MM_CollectionsViewController alloc]init];
...
-(void)headImageScrollerView{
    ...
    for(NSDictionary *dic in headPArray) {
         NSString *imageStr=[NSString stringWithFormat:@"%@img/%@",URLM,dic[@"ShopMainImg"]];
